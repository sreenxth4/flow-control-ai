...
export function TrafficMap({
  ...
  useEffect(() => {
    ...
    const resolveRoadIds = (path: string[], explicitRoadIds?: string[]): string[] => {
      if (explicitRoadIds && explicitRoadIds.length > 0) return explicitRoadIds;
      const resolved: string[] = [];
      for (let i = 0; i < path.length - 1; i++) {
        const roadId = edgeToRoadId.get(`${path[i]}->${path[i + 1]}`);
        if (roadId) resolved.push(roadId);
      }
      return resolved;
    };

    const routeRoadSet = new Set<string>(routePath && routePath.length > 1 ? resolveRoadIds(routePath) : []);
    const multiRouteRoadSets = multiRoutePaths.map((r) => ({
      set: new Set<string>(resolveRoadIds(r.path, r.roads)),
      color: r.color,
    }));
